  ...
  --  proxy.queries:append(1,string.char(proxy.COM_QUERY)..server_check,{resultset_is_needed = true})
    ...
    local InsertDeplHis = "insert into deployment_history(release_name,"..
        ...
        --      "   ) as oid_deployment_env "..
              ...
              "   ,ifnull(@ticket_name,'DUMMYTICKET') as ticket_name "..
     ...
     -- add summary into
    local TicketName="select @TicketName:=ifnull(@ticket_name,'DUMMYTICKET');"
    local LastTicketName="select @LastTicketName:=@TicketName;"
    local TicketStartTimePoint="select if(@TicketName<>ifnull(@LastTicketName,''), (select @TicketStartTimePoint:=@begintime),'');"
    print('test.... @TicketName')
   
    local PrepareSummaryData ="insert into tmpdb.test_deploy select @Summary_release_name:=release_name,"..
                                "  @Summary_db_server:=db_server,"..
                                "  @Summary_db_port:=db_port,"..
                                "  @Summry_ticket_name:=ticket_name,"..
                                "  @Summary_version_info:=group_concat( distinct"..
                                "  concat(left(script_name,locate('_',script_name)-1),database_name)) "..
                                " ,@Summary_start_datetime:=( select @Btime:=min(start_datetime) from deploymentdb.deployment_history_all "..
                                "  where start_datetime>=@TicketStartTimePoint and ticket_name=ifnull(@ticket_name,'DUMMYTICKET') and unit_number="..
                                "  (select min(unit_number) from deploymentdb.deployment_history_all where ticket_name=ifnull(@ticket_name,'DUMMYTICKET'))) ,"..
                                "  @Summary_execution_time:=timestampdiff(second,@Btime,max(finish_datetime)) as execution_time"..
                                "  from deploymentdb.deployment_history_all "..
                                "  where 1=1"..
                                 "  and ticket_name=@TicketName"..
                                "   and start_datetime >= @TicketStartTimePoint"..
     ...
     local InsertSummaryData = "insert into deploymentdb.deployment_history_summary("..
                                "release_name,"..
                                "db_server,"..
                                "db_port,"..
                                "ticket_name,"..
    ...
    local UpdateSummaryDataSuccess = " update deploymentdb.deployment_history_summary "..
                               " set deployment_status='successed'"..
                               " where ticket_name=ifnull(@ticket_name,'DUMMYTICKET')";     
    ...
    for i = 1, 1 do 
      ...
      -- 'TK_SQL_DELETE' 'TK_SQL_UPDATE' 'TK_SQL_INSERT'
        ...
        proxy.queries:append(31,string.char(proxy.COM_QUERY) .. TicketName,{resultset_is_needed = true})
        proxy.queries:append(32,string.char(proxy.COM_QUERY) .. TicketStartTimePoint,{resultset_is_needed = true})
        ...
        proxy.queries:append(50,string.char(proxy.COM_QUERY) .. PrepareSummaryData,{resultset_is_needed = true})
        proxy.queries:append(51,string.char(proxy.COM_QUERY) .. InsertSummaryData,{resultset_is_needed = true})
        proxy.queries:append(52,string.char(proxy.COM_QUERY) .. UpdateSummaryDataSuccess,{resultset_is_needed = true})
        proxy.queries:append(33,string.char(proxy.COM_QUERY) .. LastTicketName,{resultset_is_needed = true})
    ...
    if res.query_status == proxy.MYSQLD_PACKET_ERR then
      ...
      if l_Stoplog== 'NO'then
        ...
        proxy.queries:append(61,string.char(proxy.COM_QUERY) .. 'update deploymentdb.deployment_history_all set sql_state=null,finish_datetime=now() where oid_sub_deployment_history=@i_oid_dh and database_name=schema();',{resultset_is_needed = true})
            ...
            local UpdateSummaryDataFail = " update deploymentdb.deployment_history_summary "..
                               " set deployment_status='failed',execution_time=timestampdiff(second,@Btime,now()) "..
                               " where ticket_name=ifnull(@ticket_name,'DUMMYTICKET')";  
                               
        proxy.queries:append(62,string.char(proxy.COM_QUERY) .. UpdateSummaryDataFail,{resultset_is_needed = true})
