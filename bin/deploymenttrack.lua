...
function read_query(packet1)
	...
	if packet1:byte() == proxy.COM_QUERY	then
		...
		local query3 = "insert into deployment_history(release_name,"..
						" oid_deployment_env, "..
						" db_id, "..
						" ticket_name, "..
						" script_name, "..
		...
		for i = 1, 1 do 
            ...
            if token["token_name"] == 'TK_SQL_CREATE'
				...
				then
				l_Stoplog = "NO"
				proxy.queries:append(4,string.char(proxy.COM_QUERY) .. query3,{resultset_is_needed = true})
	...
	elseif inj.id == 3 then
		...
		if res.query_status == proxy.MYSQLD_PACKET_ERR then
			...
			if l_Stoplog== 'NO'then
				...
				proxy.queries:append(6,string.char(proxy.COM_QUERY) .. 'update deployment_history set sql_state=null,finish_datetime=now() where oid_deployment_history=last_insert_id();',{resultset_is_needed = true})
			
				proxy.queries:append(7,string.char(proxy.COM_QUERY) .. 'insert into deployment_error(oid_deployment_history,error_DDL,error_no,error_message,execute_timestamp) values(last_insert_id() ,"'..inj_query..'",'..res.raw:byte(2)+(res.raw:byte(3)*256)..',"'..string.format('%s', res.raw:sub(10))..'",now())',{resultset_is_needed = true})
			end
