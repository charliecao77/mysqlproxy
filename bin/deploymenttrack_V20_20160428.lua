  ...
  --  proxy.queries:append(1,string.char(proxy.COM_QUERY)..server_check,{resultset_is_needed = true})
    ...
    print('test.... @TicketName')
     ...
     local InsertSummaryData = "insert into deploymentdb.deployment_history_summary("..
                                ...
                                "  concat(left(script_name,locate('_',script_name)-1),database_name)) as version_info1"..
                                " , ( select @Btime:=min(start_datetime) from deploymentdb.deployment_history_all "..
                                "  where start_datetime>=@TicketStartTimePoint and ticket_name=ifnull(@ticket_name,'DUMMYTICKET') and unit_number="..
                                "  (select min(unit_number) from deploymentdb.deployment_history_all where ticket_name=ifnull(@ticket_name,'DUMMYTICKET'))) as start_datetime,"..
                                "  timestampdiff(second,@Btime,max(finish_datetime)) as execution_time"..
                                "  from deploymentdb.deployment_history_all as a "..
                                "  where 1=1"..
